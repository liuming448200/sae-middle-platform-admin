define('B:widget/pages/main/song/english/preview/preview.async', function(require, exports, module) {
  ...
  return ['$scope', '$stateParams', '$state', 'webservice', 'songService', 'dialogService', 'globals', 
    ...
    var language = 'english';
  	...
  	$scope.back = function () {
      if ($scope.song_id) {
        $state.go('main.song.english.edit', {
          song_id: $scope.song_id,
          back: 1
        });
      } else {
        $state.go('main.song.english.add', {
          back: 1
        });
      }
    };
    ...
    function goSongList () {
      $state.go('main.song.english.list');
    }
